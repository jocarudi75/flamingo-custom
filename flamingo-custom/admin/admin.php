...
//	Cuando el WP ejecute el Hook 'admin_menu' se llamará a la función flamingo_admin_menu
add_action( 'admin_menu', 'flamingo_admin_menu', 8, 0 );

function flamingo_admin_menu() {
	add_menu_page(
		__( 'Flamingo Address Book', 'flamingo' ),	// 	Título traducido de la página
		__( 'Flamingo', 'flamingo' ),				//	Título del menú traducido
		'flamingo_edit_contacts',					// 	capability (permisos necesarios)
		'flamingo',									//	menu_slug: cadena a la cual se dirige el enlace del menú, en caso de no existir valor para el siguiente parámetro
		'flamingo_contact_admin_page',				//	function (opcional): Función que se realizará al pulsar en el enlace. También puede ser una ruta a un archivo. De ser nulo, enlazará a la cadena escrita en «menu_slug«.
		'dashicons-feedback',						//	icon_url (opcional): url del pequeño icono (20×20) de la sección.
		28
	);
...
function flamingo_load_inbound_admin() {
	...
	if ( 'save' === $action and ! empty( $_REQUEST['post'] ) ) {
		...
		if ( ! empty( $post ) ) {
			...
			$status = isset( $_POST['inbound']['status'] ) ? $_POST['inbound']['status'] : '';
			...
			/***  jrd@uma.es  cargo el contenido de los campos editados del POST para hacer el save del inbound */
			/*	https://www.neoguias.com/if-abreviado-en-php-el-operador-ternario/ */
			$post->subject = ! empty($_POST['subject']) ? trim( $_POST['subject'] ) : $post->subject ;
			$post->from = ! empty($_POST['from']) ? trim( $_POST['from'] ) : $post->from ;
			$post->from_name = ! empty($_POST['your-name']) ? trim( $_POST['your-name'] ) : $post->from_name ;
			// Para cada uno de los campos del Flamingo_Inbound_Message se toma el valor enviado vía $_POST y se almacena antes de guardar los cambios.
			foreach ($post->fields as $nombrecampo => $value) {
				/** jrd@uma.es  Actualizo los campos del formulario  **/
				$post->fields[$nombrecampo] =  ! empty($_POST[$nombrecampo]) ? trim( $_POST[$nombrecampo] ) : $post->fields[$nombrecampo] ;
			}
			$post->save();
/*
			check_admin_referer( 'flamingo-update-contact_' . $post->id() );
...
//	Aquí se muestran los campos del mensaje de correo electrónico
//  Modificado por jrd@uma.es 
